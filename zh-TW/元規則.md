-- Meta Rules導入放置於「用戶偏好」位置（網頁版用戶），或是和Meta Rules放同一區，但在其前。
...
-- Meta Rules放置於系統提示詞、專案提示詞或對話開頭位置。不同平台最優位置可能不同，平台大多會針對開放的常駐提示詞位置偷塞針對性規則，可能影響效益，網頁版個人用法是「Meta Rules導入」放「用戶偏好」，「元規則本體」放「對話開頭」，API則是直接都置於系統提示詞區域即可。
